...
//namespace {
    ...
    bool DpStGraph::CheckOverlapOnDpStGraph(//const std::vector<const StBoundary*>& boundaries,
                                 const StGraphPoint& p1, const StGraphPoint& p2) {
        //这里直接用两点连线方向当做车辆方向，两点长度当做车辆长度，来做碰撞检测。
        Car line_car;
        line_car.width =2.1;
        line_car.RtoT = 0.8;
        double angle =BasicStruct::AngleNormalnize1(atan2(p2.point().s-p1.point().s,p2.point().t-p1.point().t));
        double length =4.2+ sqrt( (p2.point().s-p1.point().s)*(p2.point().s-p1.point().s)+
                                            (p2.point().t-p1.point().t)* (p2.point().t-p1.point().t)   );
        line_car.length = length;
        line_car.Position=RoadPoint(p1.point().t,p1.point().s,angle,0.0);
        RoadPoint co;
        if(m_obstacle.DetectGridObs(line_car,co))
            return true;
...
//路径上无障碍物，直接按照默认速度，匀速行驶。
    if (m_obstacle.GetGridObs().empty()) {
...
void DpStGraph::CalculateCostAt(const uint32_t c, const uint32_t r) {
    ...
    if (c == 1) {
        ...
        //如果有重叠 直接返回
        if (CheckOverlapOnDpStGraph( cost_cr,
                                    cost_init)) {
            return;
        }
