...
public:
    ...
    static bool LineSegOverPolygon(vector<double>vertX, vector<double>vertY,double x1,double y1,double x2,double y2);
    static bool PolygonOverPolygon(vector<double>vertX, vector<double>vertY, vector<double>_vertX, vector<double>_vertY);
    static bool LineSegOverLineSeg(double x1,double y1,double x2,double y2,double u1,double v1,double u2,double v2);
...
struct Car//车结构体
{
    ...
    double theta;   //前轮转角
    double length = 4.2;//车长
    double width = 2.2 ;//车宽

    double L;//前后轮轴距
    double RtoT = 0.8 ;//后轮距离车尾的距离
