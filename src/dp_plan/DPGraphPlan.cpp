...
//            init_sl_point_.s + std::fmax(init_point.v * 8.0, kMinSampleDistance)
    ...
    // 采样步长 = 初始速度 × 采样前视时长，要求：
    // step_length_min(默认值：8) <= 采样步长 <= step_length_max(默认值：15)//正常情况 至少8米
    ...
    while(accumulated_s<total_length)//纵向
    {
        ...
        if(accumulated_s>(total_length+step_length/2.0))//超出总长度过多
            break;
        const float kDefaultUnitL =config_.lateral_sample_offset_step;//半米一个点 默认横向偏移
        const float offset = config_.lateral_sample_offset;
...
PathPointxy DPGraphPlan::Getfinalpath(common::FrenetFramePath &lastFrenetPath) {
